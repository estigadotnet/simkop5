...
--
-- Host: localhost:3307
-- Generation Time: Feb 04, 2019 at 10:12 AM
...
(1, '201812', '3.1000', 0.00),
(2, '201812', '5.1000', 25.00),
(3, '201812', '5.2000', 0.00),
(4, '201812', '5.3000', 0.00),
(5, '201812', '5.4000', 18.00),
...
(12, '201812', '4.7000', 0.00),
(13, '201812', '6.1000', 0.00),
(16, '201901', '3.1000', 233.00),
...
('1.1003', 'KAS BANK - BCA SURABAYA', '2,200,000.00'),
...
('1.2003', 'PINJAMAN BERJANGKA & ANGSURAN', '-1,734,000.00'),
...
('2.9000', 'SHU BULAN BERJALAN', '466,000.00'),
('', '', '<strong>466,000.00</stro'),
('', '', ''),
('', '', '<strong>0.00</strong>');
...
('3.1000', 'PENDAPATAN BUNGA PINJAMAN', '466,000.00'),
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v05_labarugi`  AS  select `a`.`Tahun_Bulan` AS `Tahun_Bulan`,`a`.`id` AS `id`,`a`.`NoUrut` AS `NoUrut`,(case when isnull(sum(`b`.`Debet`)) then 0 else sum(`b`.`Debet`) end) AS `debet`,(case when isnull(sum(`b`.`Kredit`)) then 0 else sum(`b`.`Kredit`) end) AS `kredit` from (`v04_lrper` `a` left join `t10_jurnal` `b` on(((`a`.`Tahun_Bulan` = `b`.`Periode`) and (`a`.`id` = convert(`b`.`Rekening` using utf8))))) group by `a`.`Tahun_Bulan`,`a`.`NoUrut`,`a`.`id` order by `a`.`Tahun_Bulan`,`a`.`NoUrut`,`a`.`id` ;
