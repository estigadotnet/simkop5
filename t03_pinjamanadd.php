...
class ct03_pinjaman_add extends ct03_pinjaman {
	...
	function ValidateForm() {
		...
		if (in_array("t08_pinjamanpotongan", $DetailTblVar) && $GLOBALS["t08_pinjamanpotongan"]->DetailAdd) {
	...
	function AddRow($rsold = NULL) {
		...
		if ($AddRow) {
			...
			if (in_array("t08_pinjamanpotongan", $DetailTblVar) && $GLOBALS["t08_pinjamanpotongan"]->DetailAdd) {
				$GLOBALS["t08_pinjamanpotongan"]->pinjaman_id->setSessionValue($this->id->CurrentValue); // Set master key
				if (!isset($GLOBALS["t08_pinjamanpotongan_grid"])) $GLOBALS["t08_pinjamanpotongan_grid"] = new ct08_pinjamanpotongan_grid(); // Get detail page object
				$Security->LoadCurrentUserLevel($this->ProjectID . "t08_pinjamanpotongan"); // Load user level of detail table
				$AddRow = $GLOBALS["t08_pinjamanpotongan_grid"]->GridInsert();
				$Security->LoadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
				if (!$AddRow)
					$GLOBALS["t08_pinjamanpotongan"]->pinjaman_id->setSessionValue(""); // Clear master key if insert failed
			}
		}
	...
	function SetUpDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("t08_pinjamanpotongan", $DetailTblVar)) {
				...
				if ($GLOBALS["t08_pinjamanpotongan_grid"]->DetailAdd) {
					if ($this->CopyRecord)
						$GLOBALS["t08_pinjamanpotongan_grid"]->CurrentMode = "copy";
					else
						$GLOBALS["t08_pinjamanpotongan_grid"]->CurrentMode = "add";
					$GLOBALS["t08_pinjamanpotongan_grid"]->CurrentAction = "gridadd";
...
<?php
$wrkonchange = trim("ew_UpdateOpt.call(this); " . @$t03_pinjaman->nasabah_id->EditAttrs["onchange"]);
...
<input type="hidden" data-table="t03_pinjaman" data-field="x_nasabah_id" data-value-separator="<?php echo $t03_pinjaman->nasabah_id->DisplayValueSeparatorAttribute() ?>" name="x_nasabah_id" id="x_nasabah_id" value="<?php echo ew_HtmlEncode($t03_pinjaman->nasabah_id->CurrentValue) ?>"<?php echo $wrkonchange ?>>
<input type="hidden" name="q_x_nasabah_id" id="q_x_nasabah_id" value="<?php echo $t03_pinjaman->nasabah_id->LookupFilterQuery(true) ?>">
<script type="text/javascript">
ft03_pinjamanadd.CreateAutoSuggest({"id":"x_nasabah_id","forceSelect":false});
</script>
</span>
...
?>
<?php
	if (in_array("t08_pinjamanpotongan", explode(",", $t03_pinjaman->getCurrentDetailTable())) && $t08_pinjamanpotongan->DetailAdd) {
		if ($FirstActiveDetailTable == "" || $FirstActiveDetailTable == "t08_pinjamanpotongan") {
			$FirstActiveDetailTable = "t08_pinjamanpotongan";
		}
?>
		<li<?php echo $t03_pinjaman_add->DetailPages->TabStyle("t08_pinjamanpotongan") ?>><a href="#tab_t08_pinjamanpotongan" data-toggle="tab"><?php echo $Language->TablePhrase("t08_pinjamanpotongan", "TblCaption") ?></a></li>
<?php
	}
?>
	</ul>
...
?>
		<div class="tab-pane<?php echo $t03_pinjaman_add->DetailPages->PageStyle("t08_pinjamanpotongan") ?>" id="tab_t08_pinjamanpotongan">
