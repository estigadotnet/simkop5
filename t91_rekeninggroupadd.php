...
$t91_rekeninggroup_add = NULL; // Initialize page object first

class ct91_rekeninggroup_add extends ct91_rekeninggroup {
	...
	// Page object name
	var $PageObjName = 't91_rekeninggroup_add';
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "t91_rekeninggroupview.php")
	...
	function LoadDefaultValues() {
		...
		$this->NamaGroup->CurrentValue = NULL;
		$this->NamaGroup->OldValue = $this->NamaGroup->CurrentValue;
	...
	function AddRow($rsold = NULL) {
		...
		// NamaGroup
		$this->NamaGroup->SetDbValueDef($rsnew, $this->NamaGroup->CurrentValue, "", FALSE);
...
// Create page object
if (!isset($t91_rekeninggroup_add)) $t91_rekeninggroup_add = new ct91_rekeninggroup_add();

// Page init
$t91_rekeninggroup_add->Page_Init();

// Page main
$t91_rekeninggroup_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$t91_rekeninggroup_add->Page_Render();
?>
...
var CurrentForm = ft91_rekeninggroupadd = new ew_Form("ft91_rekeninggroupadd", "add");

// Validate form
ft91_rekeninggroupadd.Validate = function() {
...
ft91_rekeninggroupadd.Form_CustomValidate = 
...
ft91_rekeninggroupadd.ValidateRequired = true;
<?php } else { ?>
ft91_rekeninggroupadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ft91_rekeninggroupadd.Lists["x_Posisi"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
ft91_rekeninggroupadd.Lists["x_Posisi"].Options = <?php echo json_encode($t91_rekeninggroup->Posisi->Options()) ?>;
ft91_rekeninggroupadd.Lists["x_Laporan"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
ft91_rekeninggroupadd.Lists["x_Laporan"].Options = <?php echo json_encode($t91_rekeninggroup->Laporan->Options()) ?>;
...
<?php if (!$t91_rekeninggroup_add->IsModal) { ?>
<div class="ewToolbar">
...
<?php } ?>
<?php $t91_rekeninggroup_add->ShowPageHeader(); ?>
<?php
$t91_rekeninggroup_add->ShowMessage();
?>
<form name="ft91_rekeninggroupadd" id="ft91_rekeninggroupadd" class="<?php echo $t91_rekeninggroup_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($t91_rekeninggroup_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $t91_rekeninggroup_add->Token ?>">
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $t91_rekeninggroup_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
ft91_rekeninggroupadd.Init();
</script>
<?php
$t91_rekeninggroup_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$t91_rekeninggroup_add->Page_Terminate();
?>
