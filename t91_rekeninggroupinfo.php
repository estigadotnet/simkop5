...
$t91_rekeninggroup = NULL;

//
// Table class for t91_rekeninggroup
//
class ct91_rekeninggroup extends cTable {
	var $id;
	var $Kode;
	var $NamaGroup;
	var $Posisi;
	...
	function __construct() {
		...
		$this->TableVar = 't91_rekeninggroup';
		$this->TableName = 't91_rekeninggroup';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`t91_rekeninggroup`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new cField('t91_rekeninggroup', 't91_rekeninggroup', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->id->Sortable = TRUE; // Allow sort
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// Kode
		$this->Kode = new cField('t91_rekeninggroup', 't91_rekeninggroup', 'x_Kode', 'Kode', '`Kode`', '`Kode`', 200, -1, FALSE, '`Kode`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Kode->Sortable = TRUE; // Allow sort
		$this->fields['Kode'] = &$this->Kode;

		// NamaGroup
		$this->NamaGroup = new cField('t91_rekeninggroup', 't91_rekeninggroup', 'x_NamaGroup', 'NamaGroup', '`NamaGroup`', '`NamaGroup`', 200, -1, FALSE, '`NamaGroup`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->NamaGroup->Sortable = TRUE; // Allow sort
		$this->fields['NamaGroup'] = &$this->NamaGroup;

		// Posisi
		$this->Posisi = new cField('t91_rekeninggroup', 't91_rekeninggroup', 'x_Posisi', 'Posisi', '`Posisi`', '`Posisi`', 202, -1, FALSE, '`Posisi`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'RADIO');
		$this->Posisi->Sortable = TRUE; // Allow sort
		$this->Posisi->OptionCount = 2;
		$this->fields['Posisi'] = &$this->Posisi;

		// Laporan
		$this->Laporan = new cField('t91_rekeninggroup', 't91_rekeninggroup', 'x_Laporan', 'Laporan', '`Laporan`', '`Laporan`', 202, -1, FALSE, '`Laporan`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'RADIO');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`t91_rekeninggroup`";
	}
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "t91_rekeninggrouplist.php";
		}
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("t91_rekeninggroupview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("t91_rekeninggroupview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "t91_rekeninggroupadd.php?" . $this->UrlParm($parm);
		else
			$url = "t91_rekeninggroupadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("t91_rekeninggroupedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("t91_rekeninggroupadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("t91_rekeninggroupdelete.php", $this->UrlParm());
	}
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		$this->NamaGroup->EditValue = $this->NamaGroup->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->NamaGroup->Exportable) $Doc->ExportCaption($this->NamaGroup);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->NamaGroup->Exportable) $Doc->ExportField($this->NamaGroup);
