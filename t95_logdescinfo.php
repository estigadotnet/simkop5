...
//
// Table class for t95_logdesc
//
class ct95_logdesc extends cTable {
	...
	function __construct() {
		...
		$this->TableVar = 't95_logdesc';
		$this->TableName = 't95_logdesc';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`t95_logdesc`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new cField('t95_logdesc', 't95_logdesc', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->id->Sortable = TRUE; // Allow sort
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// log_id
		$this->log_id = new cField('t95_logdesc', 't95_logdesc', 'x_log_id', 'log_id', '`log_id`', '`log_id`', 3, -1, FALSE, '`log_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function GetMasterFilter() {
		...
		// Master filter
		$sMasterFilter = "";
		if ($this->getCurrentMasterTable() == "t94_log") {
			if ($this->log_id->getSessionValue() <> "")
				$sMasterFilter .= "`id`=" . ew_QuotedValue($this->log_id->getSessionValue(), EW_DATATYPE_NUMBER, "DB");
			else
				return "";
		}
	...
	function GetDetailFilter() {
		...
		if ($this->getCurrentMasterTable() == "t94_log") {
			if ($this->log_id->getSessionValue() <> "")
				$sDetailFilter .= "`log_id`=" . ew_QuotedValue($this->log_id->getSessionValue(), EW_DATATYPE_NUMBER, "DB");
			else
				return "";
		}
	...
	// Master filter
	function SqlMasterFilter_t94_log() {
		return "`id`=@id@";
	}

	// Detail filter
	function SqlDetailFilter_t94_log() {
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`t95_logdesc`";
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("t95_logdescview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("t95_logdescview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "t95_logdescadd.php?" . $this->UrlParm($parm);
		else
			$url = "t95_logdescadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("t95_logdescedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("t95_logdescadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("t95_logdescdelete.php", $this->UrlParm());
	}

	// Add master url
	function AddMasterUrl($url) {
		if ($this->getCurrentMasterTable() == "t94_log" && strpos($url, EW_TABLE_SHOW_MASTER . "=") === FALSE) {
			$url .= (strpos($url, "?") !== FALSE ? "&" : "?") . EW_TABLE_SHOW_MASTER . "=" . $this->getCurrentMasterTable();
			$url .= "&fk_id=" . urlencode($this->log_id->CurrentValue);
		}
		return $url;
	}
