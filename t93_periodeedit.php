...
$t93_periode_edit = NULL; // Initialize page object first

class ct93_periode_edit extends ct93_periode {
	...
	// Page object name
	var $PageObjName = 't93_periode_edit';
	...
	function Page_Main() {
		...
		if ($this->TotalRecs <= 0) { // No record found
			...
			$this->Page_Terminate("t93_periodelist.php"); // Return to list page
		...
		switch ($this->CurrentAction) {
			...
			Case "U": // Update
				...
				if (ew_GetPageName($sReturnUrl) == "t93_periodelist.php")
	...
	function SetupBreadcrumb() {
		...
		$Breadcrumb->Add("list", $this->TableVar, $this->AddMasterUrl("t93_periodelist.php"), "", $this->TableVar, TRUE);
		$PageId = "edit";
...
// Create page object
if (!isset($t93_periode_edit)) $t93_periode_edit = new ct93_periode_edit();

// Page init
$t93_periode_edit->Page_Init();

// Page main
$t93_periode_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$t93_periode_edit->Page_Render();
?>
...
var CurrentForm = ft93_periodeedit = new ew_Form("ft93_periodeedit", "edit");

// Validate form
ft93_periodeedit.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $t93_periode->Bulan->FldCaption(), $t93_periode->Bulan->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_Bulan");
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($t93_periode->Bulan->FldErrMsg()) ?>");
			elm = this.GetElements("x" + infix + "_Tahun");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $t93_periode->Tahun->FldCaption(), $t93_periode->Tahun->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_Tahun");
			if (elm && !ew_CheckInteger(elm.value))
...
ft93_periodeedit.Form_CustomValidate = 
...
ft93_periodeedit.ValidateRequired = true;
<?php } else { ?>
ft93_periodeedit.ValidateRequired = false; 
<?php } ?>
...
<?php } ?>
<?php $t93_periode_edit->ShowPageHeader(); ?>
<?php
$t93_periode_edit->ShowMessage();
?>
<form name="ft93_periodeedit" id="ft93_periodeedit" class="<?php echo $t93_periode_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($t93_periode_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $t93_periode_edit->Token ?>">
<?php } ?>
...
<?php if ($t93_periode->Bulan->Visible) { // Bulan ?>
	<div id="r_Bulan" class="form-group">
		...
		<div class="col-sm-10"><div<?php echo $t93_periode->Bulan->CellAttributes() ?>>
<span id="el_t93_periode_Bulan">
<input type="text" data-table="t93_periode" data-field="x_Bulan" name="x_Bulan" id="x_Bulan" size="30" placeholder="<?php echo ew_HtmlEncode($t93_periode->Bulan->getPlaceHolder()) ?>" value="<?php echo $t93_periode->Bulan->EditValue ?>"<?php echo $t93_periode->Bulan->EditAttributes() ?>>
</span>
<?php echo $t93_periode->Bulan->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($t93_periode->Tahun->Visible) { // Tahun ?>
	<div id="r_Tahun" class="form-group">
		<label id="elh_t93_periode_Tahun" for="x_Tahun" class="col-sm-2 control-label ewLabel"><?php echo $t93_periode->Tahun->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $t93_periode->Tahun->CellAttributes() ?>>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $t93_periode_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php if (!isset($t93_periode_edit->Pager)) $t93_periode_edit->Pager = new cPrevNextPager($t93_periode_edit->StartRec, $t93_periode_edit->DisplayRecs, $t93_periode_edit->TotalRecs) ?>
<?php if ($t93_periode_edit->Pager->RecordCount > 0 && $t93_periode_edit->Pager->Visible) { ?>
<div class="ewPager">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($t93_periode_edit->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $t93_periode_edit->PageUrl() ?>start=<?php echo $t93_periode_edit->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($t93_periode_edit->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $t93_periode_edit->PageUrl() ?>start=<?php echo $t93_periode_edit->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $t93_periode_edit->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($t93_periode_edit->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $t93_periode_edit->PageUrl() ?>start=<?php echo $t93_periode_edit->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($t93_periode_edit->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $t93_periode_edit->PageUrl() ?>start=<?php echo $t93_periode_edit->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $t93_periode_edit->Pager->PageCount ?></span>
</div>
<?php } ?>
<div class="clearfix"></div>
<?php } ?>
...
<script type="text/javascript">
ft93_periodeedit.Init();
</script>
<?php
$t93_periode_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$t93_periode_edit->Page_Terminate();
?>
