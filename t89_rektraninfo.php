...
//
// Table class for t89_rektran
//
class ct89_rektran extends cTable {
	...
	function __construct() {
		...
		$this->TableVar = 't89_rektran';
		$this->TableName = 't89_rektran';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`t89_rektran`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new cField('t89_rektran', 't89_rektran', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->id->Sortable = TRUE; // Allow sort
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// KodeTransaksi
		$this->KodeTransaksi = new cField('t89_rektran', 't89_rektran', 'x_KodeTransaksi', 'KodeTransaksi', '`KodeTransaksi`', '`KodeTransaksi`', 200, -1, FALSE, '`KodeTransaksi`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->KodeTransaksi->Sortable = TRUE; // Allow sort
		$this->fields['KodeTransaksi'] = &$this->KodeTransaksi;

		// JenisTransaksi
		$this->JenisTransaksi = new cField('t89_rektran', 't89_rektran', 'x_JenisTransaksi', 'JenisTransaksi', '`JenisTransaksi`', '`JenisTransaksi`', 200, -1, FALSE, '`JenisTransaksi`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->JenisTransaksi->Sortable = TRUE; // Allow sort
		$this->fields['JenisTransaksi'] = &$this->JenisTransaksi;

		// DebetRekening
		$this->DebetRekening = new cField('t89_rektran', 't89_rektran', 'x_DebetRekening', 'DebetRekening', '`DebetRekening`', '`DebetRekening`', 200, -1, FALSE, '`DebetRekening`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		$this->DebetRekening->Sortable = TRUE; // Allow sort
		$this->DebetRekening->UsePleaseSelect = TRUE; // Use PleaseSelect by default
		$this->DebetRekening->PleaseSelectText = $Language->Phrase("PleaseSelect"); // PleaseSelect text
		$this->fields['DebetRekening'] = &$this->DebetRekening;

		// KreditRekening
		$this->KreditRekening = new cField('t89_rektran', 't89_rektran', 'x_KreditRekening', 'KreditRekening', '`KreditRekening`', '`KreditRekening`', 200, -1, FALSE, '`KreditRekening`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		$this->KreditRekening->Sortable = TRUE; // Allow sort
		$this->KreditRekening->UsePleaseSelect = TRUE; // Use PleaseSelect by default
		$this->KreditRekening->PleaseSelectText = $Language->Phrase("PleaseSelect"); // PleaseSelect text
		$this->fields['KreditRekening'] = &$this->KreditRekening;
	}
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`t89_rektran`";
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("t89_rektranview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("t89_rektranview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "t89_rektranadd.php?" . $this->UrlParm($parm);
		else
			$url = "t89_rektranadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("t89_rektranedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("t89_rektranadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("t89_rektrandelete.php", $this->UrlParm());
	}
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		$this->JenisTransaksi->EditValue = $this->JenisTransaksi->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->JenisTransaksi->Exportable) $Doc->ExportCaption($this->JenisTransaksi);
					if ($this->DebetRekening->Exportable) $Doc->ExportCaption($this->DebetRekening);
					if ($this->KreditRekening->Exportable) $Doc->ExportCaption($this->KreditRekening);
				} else {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->JenisTransaksi->Exportable) $Doc->ExportField($this->JenisTransaksi);
						if ($this->DebetRekening->Exportable) $Doc->ExportField($this->DebetRekening);
						if ($this->KreditRekening->Exportable) $Doc->ExportField($this->KreditRekening);
					} else {
