...
$t90_rektran_add = NULL; // Initialize page object first

class ct90_rektran_add extends ct90_rektran {
	...
	// Page object name
	var $PageObjName = 't90_rektran_add';
		...
		switch ($this->CurrentAction) {
			...
			case "C": // Copy an existing record
				if (!$this->LoadRow()) { // Load record based on key
					...
					$this->Page_Terminate("t90_rektranlist.php"); // No matching record, return to list
				}
				break;
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "t90_rektranlist.php")
						$sReturnUrl = $this->AddMasterUrl($sReturnUrl); // List page, return to list page with correct master key if necessary
					elseif (ew_GetPageName($sReturnUrl) == "t90_rektranview.php")
	...
	function LoadDefaultValues() {
		...
		$this->NamaTransaksi->CurrentValue = NULL;
		$this->NamaTransaksi->OldValue = $this->NamaTransaksi->CurrentValue;
		$this->KodeRekening->CurrentValue = NULL;
		$this->KodeRekening->OldValue = $this->KodeRekening->CurrentValue;
	}
	...
	function LoadFormValues() {
		...
		if (!$this->NamaTransaksi->FldIsDetailKey) {
			$this->NamaTransaksi->setFormValue($objForm->GetValue("x_NamaTransaksi"));
		}
		if (!$this->KodeRekening->FldIsDetailKey) {
			$this->KodeRekening->setFormValue($objForm->GetValue("x_KodeRekening"));
		}
	}
	...
	function RestoreFormValues() {
		...
		$this->NamaTransaksi->CurrentValue = $this->NamaTransaksi->FormValue;
		$this->KodeRekening->CurrentValue = $this->KodeRekening->FormValue;
	}
	...
	function RenderRow() {
		...
		$this->KodeRekening->ViewCustomAttributes = "";
			...
			if (trim(strval($this->KodeRekening->CurrentValue)) == "") {
				$sFilterWrk = "0=1";
			} else {
				$sFilterWrk = "`id`" . ew_SearchString("=", $this->KodeRekening->CurrentValue, EW_DATATYPE_STRING, "");
			}
			...
			$this->KodeRekening->EditValue = $arwrk;
	...
	function AddRow($rsold = NULL) {
		...
		// NamaTransaksi
		$this->NamaTransaksi->SetDbValueDef($rsnew, $this->NamaTransaksi->CurrentValue, "", FALSE);

		// KodeRekening
		$this->KodeRekening->SetDbValueDef($rsnew, $this->KodeRekening->CurrentValue, "", FALSE);
...
// Create page object
if (!isset($t90_rektran_add)) $t90_rektran_add = new ct90_rektran_add();

// Page init
$t90_rektran_add->Page_Init();

// Page main
$t90_rektran_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$t90_rektran_add->Page_Render();
?>
...
var CurrentForm = ft90_rektranadd = new ew_Form("ft90_rektranadd", "add");

// Validate form
ft90_rektranadd.Validate = function() {
...
ft90_rektranadd.Form_CustomValidate = 
...
ft90_rektranadd.ValidateRequired = true;
<?php } else { ?>
ft90_rektranadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ft90_rektranadd.Lists["x_KodeRekening"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_id","x_rekening","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"t91_rekening"};
...
<?php if (!$t90_rektran_add->IsModal) { ?>
<div class="ewToolbar">
...
<?php } ?>
<?php $t90_rektran_add->ShowPageHeader(); ?>
<?php
$t90_rektran_add->ShowMessage();
?>
<form name="ft90_rektranadd" id="ft90_rektranadd" class="<?php echo $t90_rektran_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($t90_rektran_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $t90_rektran_add->Token ?>">
<?php } ?>
...
<?php if ($t90_rektran->KodeRekening->Visible) { // KodeRekening ?>
	<div id="r_KodeRekening" class="form-group">
		<label id="elh_t90_rektran_KodeRekening" for="x_KodeRekening" class="col-sm-2 control-label ewLabel"><?php echo $t90_rektran->KodeRekening->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $t90_rektran->KodeRekening->CellAttributes() ?>>
<span id="el_t90_rektran_KodeRekening">
<select data-table="t90_rektran" data-field="x_KodeRekening" data-value-separator="<?php echo $t90_rektran->KodeRekening->DisplayValueSeparatorAttribute() ?>" id="x_KodeRekening" name="x_KodeRekening"<?php echo $t90_rektran->KodeRekening->EditAttributes() ?>>
<?php echo $t90_rektran->KodeRekening->SelectOptionListHtml("x_KodeRekening") ?>
</select>
<input type="hidden" name="s_x_KodeRekening" id="s_x_KodeRekening" value="<?php echo $t90_rektran->KodeRekening->LookupFilterQuery() ?>">
</span>
<?php echo $t90_rektran->KodeRekening->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div>
<?php if (!$t90_rektran_add->IsModal) { ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $t90_rektran_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
ft90_rektranadd.Init();
</script>
<?php
$t90_rektran_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$t90_rektran_add->Page_Terminate();
?>
